...
public class Window extends JApplet
{
	...
	/**
	 * Creates a Window with the given width, height, and name.
	 * 
	 * @param width - the width of the window
	 * @param height - the height of the window
	 ...
	 * @return an object to represent this window instance
	 */
	public static WindowInstance size(int width, int height, String string)
	{
	...
	{
		if (min > max) return (int) (max + Math.random() * (min - max + 1));
		else return (int) (min + Math.random() * (max - min + 1));
	}
	...
	public static class key
	{
		...
		public static boolean pressed(String key)
		{
			...
			else if (keyMap.containsKey(key)) return Window.getInstanceFromThread().isVirtualKeyPressed(keyMap.get(key));
			else if (key.length() > 0) return Window.key.pressed(key.charAt(0));
			return false;
		}
	...
	public static class out
	{
		...
		public static void rectangle(int x, int y, int width, int height, double angle)
		{
			...
			Window.out.polygon(x + rotatedX(-width / 2, -height / 2, angle), y + rotatedY(-width / 2, -height / 2, angle), x + rotatedX(-width / 2, height / 2, angle), y + rotatedY(-width / 2, height / 2, angle), x + rotatedX(width / 2, height / 2, angle), y + rotatedY(width / 2, height / 2, angle), x + rotatedX(width / 2, -height / 2, angle), y + rotatedY(width / 2, -height / 2, angle));
		}
		...
		{
			if (!initialized) Window.size(defaultWidth, defaultHeight);
			Window.getInstanceFromThread().setColor(red, green, blue, 255);
		...
		{
			if (!initialized) Window.size(defaultWidth, defaultHeight);
			Window.getInstanceFromThread().setColor(0, 0, 0, 255);
			Window.getInstanceFromThread().fillRect(0, 0, Window.width, Window.height);
		}
		...
		public static void color(String color)
		{
			...
			if (color != null)
			{
				...
				if (color.indexOf('#') == 0 && color.length() == 7)
				{
					try
					{
						Window.getInstanceFromThread().setColor(Integer.valueOf(color.substring(1, 3), 16), Integer.valueOf(color.substring(3, 5), 16), Integer.valueOf(color.substring(5, 7), 16), 255);
					}
				...
				else if (colorMap.containsKey(color))
				{
					...
					Window.getInstanceFromThread().setColor((c >> 16) & 0xff, (c >> 8) & 0xff, c & 0xff, 255);
				}
				else Window.getInstanceFromThread().setColor(0, 0, 0, 255);
			}
		}
		...
		/**
		 * Sets the color to the given RGB value (red, green, and blue) with alpha
		 * 
		 * @param red - red component
		 * @param green - green component
		 * @param blue - blue component
		...
		{
			if (!initialized) Window.size(defaultWidth, defaultHeight);
			Window.getInstanceFromThread().setColor(red, green, blue, alpha);
		}
		...
		{
			if (!initialized) Window.size(defaultWidth, defaultHeight);
			Window.getInstanceFromThread().setColor((int) (Math.random() * 256), (int) (Math.random() * 256), (int) (Math.random() * 256), 255);
		}
	...
	private static class error
	{
		...
		{
			System.err.println("Could not start a mesh at port " + port + ". " + "Try closing all open Window instances, then try again.");
		}
	...
	public static synchronized WindowInstance getInstanceFromThread()
	{
		if (!initialized) Window.size(defaultWidth, defaultHeight);
		WindowInstance instance = instanceMap.get();
	...
	private KeyListener keyListener = new KeyAdapter()
	{
		...
		public void keyPressed(KeyEvent e)
		{
			...
			if (c >= 0 && c < keyPressed.length) keyPressed[c] = keyTyped[c] = true;
			int vk = e.getKeyCode();
			if (vk >= 0 && vk < virtualKeyPressed.length) virtualKeyPressed[vk] = virtualKeyTyped[vk] = true;
		}
	...
	private MouseListener mouseListener = new MouseAdapter()
	{
		...
		{
			if (windowWidth == 0 || windowHeight == 0) return; // no display
																 // window yet
			mouseClickX = (stretchToFit ? (int) (0.5 + me.getX() * bufferSize.width / (double) windowWidth) : me.getX() - paintAtX);
			mouseClickY = (stretchToFit ? (int) (0.5 + me.getY() * bufferSize.height / (double) windowHeight) : me.getY() - paintAtY);
	...
	private MouseMotionListener mouseMotionListener = new MouseMotionAdapter()
	{
		...
		public void mouseMoved(MouseEvent me)
		{
			...
			mouseX = (stretchToFit ? (int) (0.5 + me.getX() * bufferSize.width / (double) windowWidth) : me.getX() - paintAtX);
			mouseY = (stretchToFit ? (int) (0.5 + me.getY() * bufferSize.height / (double) windowHeight) : me.getY() - paintAtY);
	...
	private static void initialize()
	{
		...
		keyMap.put("hyphen", KeyEvent.VK_MINUS);
